...
-  Connection header contains token Upgrade (case-insensitive, comma-separated)

-  Upgrade header equals websocket (case-insensitive)

-  Sec-WebSocket-Version equals 13

-  Sec-WebSocket-Key is present and looks like base64 (don’t over-validate initially)

(Optionally)


-  Check Origin if you care about CSRF-like risks (can skip for local learning)

2. Compute accept key

-  accept = base64( sha1( key + GUID ) )
	- GUID is constant: 258EAFA5-E914-47DA-95CA-C5AB0DC85B11


3. Hijack and reply 101

-  Use http.Hijacker to obtain:
	- underlying net.Conn

	- *bufio.ReadWriter (or reader/writer)


-  Write response exactly (CRLF matters):
...
-  From this point on, do not use http.ResponseWriter for this connection
...
-  For now: if you see something unsupported (e.g. binary 0x2), either ignore or close with protocol error.
...
Option A (simplest): reject fragmentation


-  If fin == false, close connection with protocol error.

This usually still works for simple browser messages (many will be single-frame), but fragmentation can happen.

Option B (still manageable): support basic fragmentation for text
...
“Must-have” compatibility items for browsers


If you do only the essentials, prioritize these:


1.  Correct handshake (Sec-WebSocket-Accept)

2.  Correct frame parsing

3.  Client frames are masked → you must unmask

4.  Handle ping (reply pong)

5.  Handle close correctly

6.  Enforce single-writer per connection
